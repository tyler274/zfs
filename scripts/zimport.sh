...
SRC_TAGS="master zfs-0.6.2 zfs-0.6.1"
POOL_TAGS="master zol-0.6.2 zol-0.6.1 zevo-1.1.1"
...
while getopts 'hvkr:s:i:p:f:?' OPTION; do
	case $OPTION in
	h)
		usage
		exit 1
...
pool_create() {
	...
	test "$POOL_TAG" != "installed" && pushd $POOL_DIR_SRC >/dev/null
	...
	$ZFS_SH -u || fail
	test "$POOL_TAG" != "installed" && popd >/dev/null
}
...
for TAG in $POOL_TAGS; do
	...
	for TAG in $SRC_TAGS; do
		...
		src_set_vars $TAG
		test "$TAG" != "installed" && pushd $ZFS_DIR >/dev/null
		...
		$ZFS_SH -u || fail
		test "$TAG" != "installed" && popd >/dev/null
	done
	printf "\n"
done
